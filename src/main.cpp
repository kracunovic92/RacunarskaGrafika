...
int main() {
    ...
    // --------------------
    GLFWwindow *window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Ship Movement Simulation", NULL, NULL);
    if (window == NULL) {
    ...
    Shader shipShader("resources/shaders/shipL.vs", "resources/shaders/shipL.fs");
    ...
    Model shipModel("resources/objects/Shiiip/saberncc61947.obj");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if(ssaoButton==true){
            ...
            glBindFramebuffer(GL_FRAMEBUFFER, gBuffer);
            glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
            glm::mat4 projection = glm::perspective(glm::radians(programState->camera.Zoom), (float) SCR_WIDTH / (float) SCR_HEIGHT,
                                                    0.1f, 100.0f);
...
//            earthModel.Draw(shaderGeometryPass);
            ...
            //Saber RENDER
            model = glm::mat4(1.0f);
            model = glm::translate(model, glm::vec3(zemljaX + sin(glfwGetTime() / 2) * 15, -3.0f,
                                                    zemljaZ + cos(glfwGetTime() / 2) * 15));
            ...
            pointLight.position = glm::vec3(0.0f, -3.0f, -57.0f);
            shipShader.setVec3("pointLight.position", pointLight.position);
            shipShader.setVec3("pointLight.ambient", pointLight.ambient);
            shipShader.setVec3("pointLight.diffuse", pointLight.diffuse);
            shipShader.setVec3("pointLight.specular", pointLight.specular);
            shipShader.setFloat("pointLight.constant", pointLight.constant);
            shipShader.setFloat("pointLight.linear", pointLight.linear);
            shipShader.setFloat("pointLight.quadratic", pointLight.quadratic);
            shipShader.setFloat("material.shininess", 512.0f);
            shipShader.setVec3("viewPosition", programState->camera.Position);


            // spotLight
            shipShader.setVec3("spotLight.position", programState->camera.Position);
            shipShader.setVec3("spotLight.direction", programState->camera.Front);
            shipShader.setVec3("spotLight.ambient", 0.0f, 0.0f, 0.0f);
            shipShader.setVec3("spotLight.diffuse", 1.0f, 1.0f, 1.0f);
            shipShader.setVec3("spotLight.specular", 1.0f, 1.0f, 1.0f);
            shipShader.setFloat("spotLight.constant", 1.0f);
            shipShader.setFloat("spotLight.linear", 0.09);
            shipShader.setFloat("spotLight.quadratic", 0.032);
            shipShader.setFloat("spotLight.cutOff", glm::cos(glm::radians(12.5f)));
            shipShader.setFloat("spotLight.outerCutOff", glm::cos(glm::radians(15.0f)));
            ...
            shipShader.setBool("blinn",blinn);
    ...
    glDeleteFramebuffers(1, &skyboxVBO);
...
void DrawImGui(ProgramState *programState) {
   ...
   /* {
    ...
    {
        ImGui::Begin("Info");
        ImGui::Text("Pronadji brod pritiskom na taster X, promeni osvetljenje na taster B");
        ImGui::End();
    }
